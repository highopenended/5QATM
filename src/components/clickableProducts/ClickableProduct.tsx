...
type ClickableProductProps = {
    ...
    href: string;
    className?: string;
    alphaThreshold?: number; // optional tuning
};

export default function ClickableProduct({ src, alt, href, className, alphaThreshold = 10 }: ClickableProductProps) {
    ...
    const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        if (alpha <= alphaThreshold) {
            ...
            // 👇 forward click to whatever is underneath
    ...
    return (
        <a href={href} onClick={handleClick} className={`clickable-product ${className ?? ""}`} aria-label={alt}>
